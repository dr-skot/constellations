...
- **Multi-line bodies**: write the body to a file (e.g. `tmp/issue-body.md`) with the Write tool, then pass `--body-file <path>`. Do **not** use heredocs or `$(...)` — this repo's working style bans `$` and heredocs in Bash.
...
- **Map**: a single issue labelled `wayfinder:map`, holding the Notes / Decisions-so-far / Fog body. `gh issue create --label wayfinder:map` (body via `--body-file`).
...
- **Resolve**: `gh issue comment <n> --body-file tmp/resolution.md`, then `gh issue close <n>`, then append a context pointer (gist + link) to the map's Decisions-so-far.
